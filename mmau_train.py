...
class MMAU_TRAIN(Dataset):
    ...
    def get_data_list(self):
        if self.phase == "train":
            list_file = os.path.join(self.root_path + "/" + 'train.txt')
    ...
    def pross_video_data(self, video):
        ...
        for fid in range(len(video)):
            try:
                ...
                print(f"Warning: File {video_data} not found in pross_video_data(), skipping this sample.")
            except Exception as e:
                print(f"A exception has occurred {e} in pross_video_data().")
    ...
    def pross_map_data(self, video):
        ...
        for fid in range(len(video)):
            try:
                ...
                print(f"Warning: File {video_data} not found in pross_map_data(), skipping this sample.")
            except Exception as e:
                print(f"A exception has occurred {e} in pross_map_data().")
        video_data = np.array(video_datas, dtype=np.float32)  # 4D tensor
        video_data = rearrange(video_data, 'f w h c -> f c w h')
        return video_data
    ...
    def __getitem__(self, index):
        ...
        for bbx_file in bbx_path:
            try:
                ...
                print(f"Warning: File {bbx_file} not found in __getitem__(), skipping this sample.")
            except Exception as e:
                print(f"A exception has occurred {e} in __getitem__().")
